...
import { miniSection } from '@/utils/helper';
import { icons } from '@/constants/icons';

// Define the type for a mini-session plan
interface MiniSessionPlan {
  name: string;
  price: number;
  benefits: string[];
}
...
interface MiniSessionPlansProps {
  selectedPlan: string;
  setSelectedPlan: React.Dispatch<React.SetStateAction<string>>;
}

const MiniSessionPlans = ({ selectedPlan, setSelectedPlan }: MiniSessionPlansProps) => {
  return (
    <View>
      <Text style={styles.planSectionTitle}>Mini Session</Text>
      {miniSection.map((plan: MiniSessionPlan) => (
        <TouchableOpacity
          key={plan.name}
...
export default MiniSessionPlans;